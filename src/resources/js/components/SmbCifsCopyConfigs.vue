  ...
  <div class="component-container">
    ...
    <div class="box is-6">
      ...
      <form
        @submit="checkForm"
        method="POST"
        action="/automations/smbCifsCopy"
        ref="smbCifsCopyForm"
      >
        <div class="field">
...
export default {
  ...
  methods: {
    ...
    delete(config) {
      ...
      axios
        .delete(`/api/automations/smbCifsCopy/${config.id}`)
        .then(() => {
          this.smbCifsCopyConfigs = this.smbCifsCopyConfigs.filter((c) => c.id !== config.id);
        })
        .catch(() => {
    ...
    submitForm() {
      axios
        .post('/api/automations/smbCifsCopy', {
          name: this.name,
    ...
    checkForm(e) {
      if (this.name && this.servicename && this.user && this.password && this.remote_dest) {
        this.submitForm();
      } else {
        this.errors = [];
