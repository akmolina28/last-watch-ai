    ...
    <div class="box is-6">
      ...
      <form @submit.prevent="processForm">
        ...
        <b-field label="Topic">
          <b-input
            v-model="topic"
            placeholder="mqtt/lastwatch/alert"
            name="topic"
            required
          ></b-input>
        </b-field>
...
export default {
  ...
  methods: {
    ...
    delete(config) {
      ...
      axios
        .delete(`/api/automations/mqttPublish/${config.id}`)
        .then(() => {
          this.mqttPublishConfigs = this.mqttPublishConfigs.filter((c) => c.id !== config.id);
        })
        .catch(() => {
    ...
    processForm() {
      ...
      this.isSaving = true;
      axios
        .post('/api/automations/mqttPublish', formData)
        .then((response) => {
